...
import { loginEducator, createBatch, updateBatch, addBatchContent, deleteBatch, toggleBatchSubTab, formatQuizFromRawText, generateQuizFromRandomText, fetchFirebaseStatus, triggerFirebaseCloudSync } from '../lib/api';
import { X, ShieldCheck, Plus, Trash2, Video, FileText, BookOpen, FileCheck, LogOut, CheckCircle2, RefreshCw, Image, Edit3, Settings, Sparkles, Wand2, HelpCircle, FileQuestion, Check, AlertCircle, Eye, EyeOff, Layers, Database, Cloud } from 'lucide-react';
...
export const EducatorPortalModal: React.FC<EducatorPortalModalProps> = ({
  ...
  const handleForceCloudSync = async () => {
    ...
    try {
      const res = await triggerFirebaseCloudSync();
      if (res.success) {
        setCloudSyncMsg('Cloud Firestore synced successfully!');
        onRefreshData();
        refreshFirebaseStatus();
      } else {
        setCloudSyncMsg(res.message || 'Sync failed.');
      }
    } catch {
      setCloudSyncMsg('Cloud synchronization error.');
    } finally {
        ...
        <div className="flex-1 flex flex-col overflow-hidden bg-[#FAF6F0]">
          ...
          <div className="flex border-b border-[#E6DCCF] bg-white px-3 py-2 gap-2 overflow-x-auto scrollbar-none">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 max-w-xl mx-auto w-full">
              ...
              <div className="bg-white border border-[#E6DCCF] rounded-2xl p-3.5 shadow-xs space-y-2.5">
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-2">
                    ...
                    <div>
                      <div className="flex items-center space-x-2">
                        <h4 className="text-xs font-black text-[#382820]">Firebase Cloud Project</h4>
                        <span className="inline-flex items-center px-1.5 py-0.5 rounded-full text-[9px] font-bold bg-emerald-100 text-emerald-800 border border-emerald-300">
                          ...
                          curious-bharat-e3c65
                        </span>
                      </div>
                      <p className="text-[10px] text-[#7A6B63]">
                        Firestore DB & Storage Bucket Active & Connected
                      </p>
                    </div>
                  </div>
                  <button
                    ...
                    className="px-2.5 py-1.5 bg-[#B85B14] hover:bg-[#A04F11] text-white text-[10px] font-bold rounded-xl shadow-xs transition-all flex items-center gap-1 cursor-pointer disabled:opacity-60"
                  >
                ...
                {cloudSyncMsg && (
                  <div className="p-2 bg-emerald-50 border border-emerald-200 text-emerald-800 text-[10px] font-bold rounded-lg animate-fadeIn flex items-center justify-between">
